...
export default async function on({ cht, Exp, store, ev, is }) {
    ...
    let exconfes = `*Contoh penggunaan*\n.${cht.cmd} nama|nomor|pesan`
    ev.on({
      cmd: [
        "confess","confes",
        "menfess","menfess"
...
➮ Untuk membuka blokir bisa mengetik .confess unblock|code atau nomor telepon`)
            } catch (e) {
              console.error(e)
          ...
          case "ga": {
               ...
               let text = `*Berhasil terhubung ✅*\n\n➮ CODE: ${String(code).toUpperCase()}\n\n➮ Untuk keluar, kamu bisa ketik .confess exit`
          ...
          case "reply": {
            ...
            if(isFound.reply) return cht.reply("Pesan tersebut sudah anda balas!")
             ...
             let value;
             let balas = `
*Hallo kak👋🏻 kamu menerima balasan nihh*
💌 Code : ${trgt}
👤 User : @${sender.split("@")[0]}
───────────────────────
📝 Pesan :
...
`
          ...
          case "connect": {
            try {
              ...
              let headConfess = trgt.length >= 7 ? "Seseorang ingin terhubung dengan anda melalui chat anonymous (confess)":`Confess dengan Code *${trgt.toUpperCase()}* ingin terhubung dengan anda` 
              let message = { 
               text: `${headConfess}
...
- Untuk menjawab, anda harus mereply pesan ini!
- Kami akan menunggu balasan anda selama 5 menit
`,
               contextInfo: {
                 isForwarded: true,
                 mentionedJid: [sender]
          ...
          default:
            ...
            const msg = `
*Hallo kak👋🏻 kamu menerima Confess nih*
💌 Code : ${code}
👤 Pengirim : ${AB}
───────────────────────
📝 Pesan :
...
📌 Info
➮ Jika anda merasa terganggu silahkan abaikan pesan ini.
➮ Anda bisa melanjutkan percakapan ini dengan mengetik .${cht.cmd}connect|${code}
➮ Code ${code} akan dihapus dalam kurun waktu ${time}`;
            if(!isText){
              value = quoted ? await cht.quoted.download() : await cht.download()
              message["caption"] = msg
            } else {
...
🟢 Last Online : ${func.dateFormatter(cht.memories.lastChat, "Asia/Jakarta")}`, { mentions: [Sender.split("@")[0]+from.sender] })
    ...
    infos.about.tfenergy ??=  `*📌 Panduan melakukan transfer Energy*
...
- 🔄 Gantilah <jumlah energy> dengan angka sesuai kebutuhan.
...
⚠️ Perlu ${parseInt(nu) - cht.memories.energy} ⚡Energy lagi untuk melakukan transfer!`)
